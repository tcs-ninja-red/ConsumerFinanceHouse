...
[
    ...
    ,
    {
        ...
        cash_price: 480000,
        color: "Caviar Black;;Velvet Red;Velvet Red + Linen Beige;Switchbleade Silver;Summit White;Linen Beige;Moonbeam White + Linen Beige;Moonbeam White;aviar Black;Sanddrift Grey;Velvet Red;Linen Beige;Moonbeam White;Summit White;",
        transmission: "Manual",
        fuel_type: "Diesel",
        body_style: "SUV",
        model_year:2008,
  ...
  function getModelDescFromMake(selectedMake, isModel) {
    ...
    var a = vehicles.forEach(function(item){
        ...
        if(item.make_name == selectedMake){
            if(isModel)
                modelLst.push({'id': counter, 'value': item.model_name});
            else
                modelLst.push({'id': counter, 'value': item.description});            
                ...
                <div className="row">
                    <div className="col-sm">
                        <div className="form-group">
                            <label>
                            Make:</label>
                            <select className="form-control md"  onChange={this.handleChangeMake}>
                            <option value="0">--Select a Make--</option>
                            {this.state.makeList.map((make) => <option key={make.id} value={make.id}>{make.value}</option>)}
                            </select>                
                        </div>  
                    </div>
                    <div className="col-sm">
                    <div className="form-group">
                            <label>
                            Model:</label>
                            <select className="form-control md" onChange={this.handleChangeModel}>
                            <option value="0">--Select a Model--</option>
                                {this.state.modelList.map((model) => <option key={model.id} value={model.id}>{model.value}</option>)}
                            </select>                
                        </div>  
                    </div>
                    <div className="col-sm">
                    <div className="form-group">
                            <label>
                            Description:</label>
                            <select className="form-control md"  onChange={this.handleChangeDesc}>
                            <option value="0">--Select a Description--</option>
                                {this.state.descList.map((desc) => <option key={desc.id} value={desc.id}>{desc.value}</option>)} 
                            </select>                
                        </div>  
                    </div>
                </div>
                ...
                <div className="container card" style={{height: "300px"}}>
                    <div className="row">
                        <div className="col-md">
                            <span style={this.styles}><b>{this.state.makeValue}</b></span> 
                            <span style={this.styles}><b>{this.state.modelValue}</b></span>
                            <span style={this.styles}><b>{this.state.descValue}</b></span>
                        </div>
                    </div>
